...
async function loadSynced() {
  dot('syncing');
  const { data: tData, error: tErr } = await sb.from('todos').select('*').order('created', { ascending: false });
  const { data: aData } = await sb.from('attachments').select('*').order('created', { ascending: true });
...
async function toggleDone(id) {
  ...
  if (mode === 'guest') { saveGuest(); render(); }
    ...
    render();
    dot('syncing');
    const { error } = await sb.from('todos').update({ done: t.done, updated: new Date().toISOString() }).eq('id', id);
...
async function deleteTodo(id) {
  ...
  if (mode === 'guest') {
    ...
    dot('syncing');
    for (const a of (attMap[id] || [])) if (a.path) await sb.storage.from('attachments').remove([a.path]);
    const { error } = await sb.from('todos').delete().eq('id', id);
    if (error) { dot('err'); toast('delete failed', 'var(--danger)'); return; }
    await loadSynced();
  }
}
...
async function commitEdit(id, text, desc) {
  ...
  if (mode === 'guest') {
    ...
    dot('syncing');
    const { error } = await sb.from('todos').update({ text, description: desc.trim(), updated: new Date().toISOString() }).eq('id', id);
...
async function clearDone() {
  ...
  if (mode === 'guest') {
    ...
    dot('syncing');
    for (const id of ids) for (const a of (attMap[id]||[])) if (a.path) await sb.storage.from('attachments').remove([a.path]);
    await sb.from('todos').delete().in('id', ids);
...
async function clearDoneAttachments() {
  ...
  if (mode === 'guest') {
    ...
    for (const id of affected) {
      for (const a of (attMap[id]||[])) if (a.path) await sb.storage.from('attachments').remove([a.path]);
      await sb.from('attachments').delete().eq('todo_id', id);
    }
...
async function uploadFile(todoId, rawFile) {
  ...
  if (mode === 'guest') {
    ...
    if (dbErr) { dot('err'); toast('record failed', 'var(--danger)'); return; }
...
async function deleteAttachment(attId, todoId) {
  if (mode === 'guest') {
    ...
    dot('syncing');
    if (a.path) await sb.storage.from('attachments').remove([a.path]);
